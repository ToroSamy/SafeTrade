...
class InteractEntityListener : Listener {
    ...
    fun onInteractEntity(event: PlayerInteractEntityEvent) {
        ...
        //向发送者发送信息
        val senderMessage = HoverUtil.replacePapi(ConfigUtil.langConfig.sendSuccessSelf,sender.name,receiver.name)
        val cancelHover = HoverUtil.replacePapi(ConfigUtil.langConfig.hoverCancel,sender.name,receiver.name)
        HoverUtil.sendCommandHover(sender, HoverUtil.createCommandHover(senderMessage,"/trade cancel",cancelHover))
        //向接收者发送信息
        val acceptText = HoverUtil.replacePapi(ConfigUtil.langConfig.textAccept,sender.name,receiver.name)
        val acceptHover = HoverUtil.replacePapi(ConfigUtil.langConfig.hoverAccept,sender.name,receiver.name)

        val denyText = HoverUtil.replacePapi(ConfigUtil.langConfig.textDeny,sender.name,receiver.name)
        val denyHover = HoverUtil.replacePapi(ConfigUtil.langConfig.hoverDeny,sender.name,receiver.name)

        receiver.sendMessage(HoverUtil.replacePapi(ConfigUtil.langConfig.sendSuccessOther,sender.name,receiver.name))
